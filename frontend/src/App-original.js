...
const DocumentUpload = ({ onUploadSuccess, account, isAuthenticated }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Add metadata - ensure all required fields have valid values
      formData.append('studentName', metadata.studentName || metadata.title || 'Student Name');
      formData.append('studentId', 'STU' + Date.now());
      formData.append('ownerName', metadata.studentName || metadata.title || 'Owner Name');
      formData.append('documentType', metadata.documentType || 'certificate');
      ...
      if (response.data.success) {
        ...
        // Reset form
        setFile(null);
        setMetadata({ title: '', studentName: '', description: '', documentType: 'certificate' });
      ...
      if (error.response) {
        ...
        if (error.response.data && error.response.data.details) {
          ...
          if (Array.isArray(error.response.data.details)) {
            validationErrors = error.response.data.details.map(err => `${err.field}: ${err.message}`).join(', ');
          } else {
            validationErrors = JSON.stringify(error.response.data.details);
          }
...
function App() {
  ...
  const handleAuthenticate = async () => {
    ...
    try {
      ...
      // Get the signer from MetaMask (ethers v6 syntax)
      const provider = new ethers.BrowserProvider(window.ethereum);
      const signer = await provider.getSigner();
      
      // Authenticate with the backend
      await authService.authenticateWallet(account, signer, 'issuer');
